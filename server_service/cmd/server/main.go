...
import (
	...
	"github.com/hollyhox-21/discord_project/server_service/internal/app"
...
func main() {
	...
	pb.RegisterServerServiceServer(grpcSrv, impl)
	reflection.Register(grpcSrv)

	// Create an HTTP server object
	mux := runtime.NewServeMux()
	if err = pb.RegisterServerServiceHandlerServer(ctx, mux, impl); err != nil {
...
func initSwagger() http.Handler {
	...
	r.Get("/server.swagger.json", func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, "pkg/server/server.swagger.json")
	})
