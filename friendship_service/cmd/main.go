...
import (
	...
	"github.com/hollyhox-21/discord_project/friendship_service/internal/app"
...
func main() {
	...
	pb.RegisterFriendshipServiceServer(grpcSrv, impl)
	reflection.Register(grpcSrv)

	// Create an HTTP server object
	mux := runtime.NewServeMux()
	if err = pb.RegisterFriendshipServiceHandlerServer(ctx, mux, impl); err != nil {
...
func initSwagger() http.Handler {
	...
	r.Mount("/", v5emb.NewHandler("API Definition",
		"/docs/friendship.swagger.json",
		"/docs"))

	r.Get("/friendship.swagger.json", func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, "pkg/friendship/friendship.swagger.json")
	})
